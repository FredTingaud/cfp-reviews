...
const prepareLogin = (app, db, home) => {
    ...
    db.defaults({ cfps: [], users: [], scores: [], tags: TAGS.map(t => ({ value: t, count: 0, checked: true })), alerts: [], resets: [] }).write();
    ...
    displayAllUsers = (req, res) => {
        ...
        res.render('login/users', {
            alerts: db.get('alerts').value(),
            users: users.map(u => ({ ...u, proposals: countProposals(u), drafts: countDrafts(u), timestamp: new Date(u.timestamp).toLocaleString() })),
    ...
    return {requireAuth: requireAuth, requireAdmin: requireAdmin};
}
