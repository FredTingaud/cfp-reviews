...
const prepareReview = (app, db) => {
    ...
    app.get('/results', requireAdmin, (req, res) => {
        ...
        const selections = reviews.filter(r => r.selected==='true').length;
    ...
    app.post('/results', requireAdmin, (req, res) => {
        ...
        results.cfpId.forEach((id, i) => db.get('selection').push({
            cfpId: id,
            selected: results.selected[i],
            track: results.selected[i] === 'true' ? results.track[i] : null,
            duration: results.selected[i] === 'true' ? results.duration[i] : null
        }).write());
