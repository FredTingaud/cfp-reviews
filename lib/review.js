...
const prepareReview = (app, db) => {
    ...
    const trackStats = (req, track) => {
        const cfpdb = db.get('cfps').filter({ changed: false, finished: true, track: track }).value();
        const scores = db.get('scores');
        const bio = canViewBio(req.user);
        ...
        cfpdb.forEach(function (cfp) {
            ...
            const avg = s.map(a => a.score).reduce((p, c) => p + c, 0) / s.length;
            const med = median(s.map(a => a.score));
            const conf = s.map(a => a.confidence).reduce((p, c) => p + c, 0) / s.length;
            const w = s.reduce((p, c) => ({ s: p.s + c.score * c.confidence, c: p.c + c.confidence }), { s: 0, c: 0 });
