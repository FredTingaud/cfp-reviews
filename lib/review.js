...
const prepareReview = (app, db) => {
    ...
    const { requireAuth, requireAdmin, sendMail } = login.prepareLogin(app, db, home);
    ...
    const trackStats = (req, track) => {
        ...
        const proposed = [...new Set(db.get('scores').filter({ changed: false, trackReco: track }).map('cfpId').value())];
    ...
    app.get('/results', requireAdmin, (req, res) => {
        ...
        let reviews = [];
        cfpdb.forEach(function (cfp) {
            const author = cfp.writer.map(w => {
                const a = db.get('users').find({ userId: w.id }).value();
                return a.firstName + ' ' + a.lastName;
            }).join(', ');
            const selection = db.get('selection').find({ cfpId: cfp.index }).value();
            reviews.push({
                cfpId: cfp.index,
                title: cfp.titleOfThePresentation,
                author: author,
                accepted: selection ? selection.accepted : null,
                track: selection ? selection.track : null,
                duration: selection ? selection.duration : null
            });
        });

        const selections = reviews.filter(r => r.accepted==='true').length;

        const progress = reviews.filter(r => r.track === 'Progress').length;
        const produce = reviews.filter(r => r.track === 'Produce').length;
        const push = reviews.filter(r => r.track === 'Push').length;

        const short = reviews.filter(r => r.duration === '30 minutes').length;
        const long = reviews.filter(r => r.duration === '60 minutes').length;
    ...
    app.post('/results', requireAdmin, (req, res) => {
        ...
        db.get('selection').remove().write();
        results.cfpId.forEach((id, i) => db.get('selection').push({
            cfpId: id,
            accepted: results.accepted[i],
            track: results.accepted[i] === 'true' ? results.track[i] : null,
            duration: results.accepted[i] === 'true' ? results.duration[i] : null
        }).write());
    ...
    findBelow = (advices) => {
        return `
During the review, juries had the option to add an "author comment". Please find below these comments for your proposal:

------
${advices.map((a, i) => 'Comment ' + (i + 1) + ': ' + a).join('\n\n')}
------
`;
    };

    sendAcceptanceMail = (mail, firstName, lastName, title, duration, track, advices) => {
        sendMail(mail, '[CPPP] Talk acceptance', `Hi ${firstName} ${lastName},

We are delighted to tell you that your talk "${title}"" has been accepted for CPPP 2021 program!

It will be scheduled in the "${track}" track as a ${duration} talk.

Presentations will be between 11AM and 6PM Paris time from December 1st to December 3rd. Please tell us (by answering to this email) if you are in a different timezone or have any other constraint on the time-slot of your talk.

We will also contact you soon to check with you the setup (camera, lighting, applications...) you can use for your presentation.

If circumstances changed for you and you are no longer able to present, please tell us (by answering to this email) as soon as possible so that we can schedule someone else.
${advices.length ? findBelow(advices) : ''}
Best regards,
...
The CPPP Team`
    ...
    sendRefusalMail = async (mail, firstName, lastName, title, advices) => {
        sendMail(mail, '[CPPP] Your talk proposal for CPPP', `Hi ${firstName} ${lastName},

We regret to inform you that your proposal "${title}" was not accepted for CPPP 2021 program.

Multiple parameters were taken into account to create a program that would be as balanced as possible and this refusal does not mean that the jury saw no value in your proposal.

We hope that we will see you at CPPP.
${advices.length ? findBelow(advices) : ''}
Best regards,
...
The CPPP Team`
    ...
    app.get('/mail-results-accepted', requireAdmin, (req, res) => {
        const cfpdb = db.get('cfps').filter({ changed: false, finished: true }).value();
        cfpdb.forEach(cfp => {
            const selection = db.get('selection').find({ cfpId: cfp.index }).value();
            const advices = db.get('scores').filter({refused: false, changed: false, cfpId: cfp.index}).filter(s => s.author).value().map(s => s.author);
            if (selection.accepted === 'true') {
                cfp.writer.forEach(w => {
                    const a = db.get('users').find({ userId: w.id }).value();
                    sendAcceptanceMail(a.email, a.firstName, a.lastName, cfp.titleOfThePresentation, selection.duration, selection.track, advices);
                });
            }
        });
        res.redirect('/stats');
    ...
    app.get('/mail-results-refused', requireAdmin, (req, res) => {
        const cfpdb = db.get('cfps').filter({ changed: false, finished: true }).value();
        cfpdb.forEach(cfp => {
            const selection = db.get('selection').find({ cfpId: cfp.index }).value();
            const advices = db.get('scores').filter({refused: false, changed: false, cfpId: cfp.index}).filter(s => s.author).value().map(s => s.author);
            if (selection.accepted === 'false') {
                cfp.writer.forEach(w => {
                    const a = db.get('users').find({ userId: w.id }).value();
                    sendRefusalMail(a.email, a.firstName, a.lastName, cfp.titleOfThePresentation, advices);
                });
            }
        });
        res.redirect('/stats');
